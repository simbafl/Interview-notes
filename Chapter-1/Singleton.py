...
class Singleton1(object):
    def __new__(cls, *args, **kwargs):
        if not hasattr(cls, '_instance'):
            ori = super(Singleton1, cls)
            cls._instance = ori.__new__(cls, *args, **kwargs)
...
class Singleton2(object):
    ...
    def __new__(cls, *args, **kwargs):
        ori = super(Singleton2, cls).__new__(cls, *args, **kwargs)
        ori.__dict__ = cls._state
        return ori
...
print(id(one))          # 18410339
print(id(two))          # 18410522
print(one == two)       # False
print(one is two)       # False
# 但是one和two具有相同的（同一个）__dict__属性
print(id(one.__dict__))  # 14154322
print(id(two.__dict__))  # 14154322
